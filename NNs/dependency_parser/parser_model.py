...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden = nn.Linear(in_features=n_features * self.embed_size, 
                                         out_features=hidden_size, bias=True)
        self.dropout = nn.Dropout(dropout_prob)
        self.hidden_to_logits = nn.Linear(in_features=hidden_size, out_features=n_classes, bias=True)
    ...
    def forward(self, t):
        ...
        emb = self.embed_to_hidden(emb)
        emb = F.relu(emb)
        emb = self.dropout(emb)
        emb = self.hidden_to_logits(emb)
        return emb
